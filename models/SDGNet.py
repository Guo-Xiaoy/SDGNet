...
from models.BeltramiDiffusion import BeltramiDiffusion
...
class NgeNet(nn.Module):
    def __init__(self, config):
        ...
        # 在 NgeNet 中初始化
        self.beltrami_diffusion = BeltramiDiffusion(

            hidden_dim=256,  # 使用文章推荐的隐藏层特征维度 128
            num_layers=4,  # 使用 4 层的扩散
            dropout=0.5,  # 使用 50% 的 dropout
    ...
    def forward(self, inputs):
        ...
        # 2.2 Beltrami Diffusion
        len_src, len_tgt = stack_lengths[-1][0], stack_lengths[-1][1]
        coords_src, coords_tgt = stack_points[-1][:len_src], stack_points[-1][len_src:]
        coords = torch.cat([coords_src, coords_tgt], dim=0).transpose(0, 1)  # (3, n)
        # print("Shape of batched_feats:", batched_feats.shape)

        # 应用Beltrami Diffusion模块，传递输入特征维度
        input_dim = batched_feats.shape[1]
        batched_feats = self.beltrami_diffusion(batched_feats, coords,
                                                input_dim=input_dim)  # (batch_size, hidden_dim, num_nodes)
        # print("Shape of batched_feats:", batched_feats.shape)

        # 2.2 information interaction
        len_src, len_tgt = stack_lengths[-1][0], stack_lengths[-1][1]
        coords_src, coords_tgt = stack_points[-1][:len_src], stack_points[-1][len_src:]
        coords_src, coords_tgt = coords_src.transpose(0, 1).unsqueeze(0), coords_tgt.transpose(0, 1).unsqueeze(0)
        feats_src, feats_tgt = batched_feats[:, :, :len_src], batched_feats[:, :, len_src:]
        ...
        ol_scores_tgt = torch.matmul(torch.softmax(inner_product.transpose(0, 1) / temperature, dim=1),
                                     attn_scores_src)  # (n2, 1)
