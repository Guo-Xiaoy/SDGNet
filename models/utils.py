...
def get_rw_adj(edge_index, edge_weight=None, norm_dim=1, fill_value=0., num_nodes=None, dtype=None):
  # 自动推断节点数量
  num_nodes = maybe_num_nodes(edge_index) if num_nodes is None else num_nodes
  ...
  # 确保索引在节点数量范围内
  if indices.max() >= num_nodes:
    raise ValueError(f"Invalid index detected: indices should be in range 0 to {num_nodes - 1}, "
                     f"found range 0 to {indices.max().item()}.")
